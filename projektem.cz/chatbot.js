/* ÚSPORAMI AI CHATBOT — Herbert · Projektem.cz */
(function(){
  const API_URL = 'https://api.usporami.cz/api/chat';
  const SESSION_ID = 'ses_' + Math.random().toString(36).substr(2, 12);
  const SITE = 'projektem';

  const style = document.createElement('style');
  style.textContent = `
    .chatbot-bubble{position:fixed;bottom:24px;right:24px;z-index:9999;width:64px;height:64px;border-radius:50%;background:#00B4D8;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,.2);transition:transform .2s,box-shadow .2s;border:none;font-size:28px}
    ...
    .chatbot-header{padding:20px;background:#023047;color:#fff;display:flex;align-items:center;justify-content:space-between;flex:0 0 auto}
    .chatbot-header-info{display:flex;align-items:center;gap:12px}
    .chatbot-avatar{width:40px;height:40px;border-radius:50%;background:#00B4D8;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:800;flex:0 0 auto;color:#fff;font-family:system-ui}
    ...
    .chatbot-msg.user{background:#00B4D8;color:#fff;align-self:flex-end;border-bottom-right-radius:6px}
    .chatbot-msg strong{font-weight:700}
    .chatbot-quick{display:flex;flex-wrap:wrap;gap:8px;padding:0 20px 12px}
    .chatbot-quick button{padding:10px 16px;border:1.5px solid #e0e0e0;border-radius:99px;background:#fff;color:#1a1a2e;font-size:13px;font-weight:600;cursor:pointer;transition:all .15s;white-space:nowrap}.chatbot-quick button:hover{border-color:#00B4D8;background:#E0F7FA}
    .chatbot-input-area{padding:12px 16px;border-top:1px solid #eee;display:flex;gap:8px;align-items:center;flex:0 0 auto}
    .chatbot-input-area input{flex:1;padding:12px 16px;border:1.5px solid #e0e0e0;border-radius:99px;font-size:14px;outline:none;font-family:inherit}.chatbot-input-area input:focus{border-color:#00B4D8}
    .chatbot-input-area button{width:40px;height:40px;border-radius:50%;background:#00B4D8;color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:18px;flex:0 0 auto}
  ...
  panel.innerHTML = `
    <div class="chatbot-header">
      <div class="chatbot-header-info"><div class="chatbot-avatar">H</div><div><h4>Herbert</h4><p>Projektový poradce</p></div></div>
  ...
  bubble.addEventListener('click', () => {
    ...
    if (!opened) {
      opened = true;
      setTimeout(() => {
        addMsg('Ahoj! 👋 Jsem Herbert, projektový poradce. Pomohu vám s dokumentací, pasporty i legalizací staveb.\n\nCo potřebujete?');
        setTimeout(() => setQuick(['📐 Pasport stavby', '🏗️ Legalizace stavby', '🏠 Projekt pro stavební povolení', '📸 3D sken nemovitosti']), 400);
      }, 300);
    }
    inputField.focus();
  });
